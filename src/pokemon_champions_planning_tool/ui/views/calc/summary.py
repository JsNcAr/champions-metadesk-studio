...
visible without scrolling to the move cards."""
...
class _Direction(ft.Column):
    ...
    def __init__(self, *, align_end: bool) -> None:
        ...
        self._pct = ft.Text("", theme_style=ft.TextThemeStyle.TITLE_SMALL, weight=ft.FontWeight.W_700)
...
class MatchupBar(ft.Container):
    def __init__(self, *, store: CalcStore) -> None:
        super().__init__()
        self.store = store
    ...
    def update_from(self) -> None:
        ...
        if ready:
            ...
            self._left.show(f"{left.name} → {right.name}", best_of(results.left_vs_right), "No damaging move yet")
            self._right.show(f"{right.name} → {left.name}", best_of(results.right_vs_left), "No damaging move yet")
