...
TIPS: dict[str, tuple[str, ...]] = {
    ...
    "Calc": (
        ...
        "Rival team (the switch above the opponents) keeps teams to plan against and the \"Current battle\": Team preview (Ctrl+B) takes the six species you see and fills each with its most used tournament set; a paste or Poképaste link gives the real sets; Meta saves any tournament team from its calculator menu. Each member is coloured against the attacker, one click loads it as the Defender (your team rail then colours itself against it), and Team vs team shows every pairing.",
        "In a battle, what you set on a rival in the Defender panel (item, ability, moves, nature, stat points) is kept for that member, and its ? mark drops the fields you have seen. Saved plans only change through \"Save Defender to …\", so browsing never overwrites them.",
