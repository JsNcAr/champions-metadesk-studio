...
TIPS: dict[str, tuple[str, ...]] = {
    ...
    "Calc": (
        "The calculator reads like a versus screen: your team above the Attacker, the rival team above the Defender, the matchup bar (each side's best hit and who moves first) and the field bar under them. Click a team member to load it on its side; right-click loads it on the other side. Click a hit in the matchup bar to open that move.",
        ...
        "In Doubles a spread move (Rock Slide, Heat Wave, Earthquake) is calculated on two targets, ×0.75 each, as the \"2 targets\" chip on its card says; click the chip for a single target when only one foe is left. The opponents list and the ratings always use two targets.",
        "The field bar sets Singles/Doubles, the weather, the terrain and the rooms (Trick Room, Gravity, Magic Room, Wonder Room); a weather or terrain set by an ability says so in its tooltip. Each side's conditions (Tailwind, screens, Helping Hand, hazards, Leech Seed, Spikes) sit under that side's Pokémon: + Condition adds one, ✕ removes it. \"Activate\" next to an ability means it has already triggered (Intimidate applied, Flash Fire lit).",
        "The side panel (Ctrl+\\ shows or hides it) has three tabs. Opponents runs the attacker against every Champions species: Crushed (you OHKO first), Threat (they KO you first), Wall (four hits or more), Mitigated (you win the race), Neutral; \"Tournament sets\" gives each opponent its four most used roster moves. Rivals lists the rival team in detail. Box loads a box Pokémon as the attacker (click) or the defender (shield).",
