...
class TestTeamStore(_TeamStoreCase):
    ...
    def test_spread_validation_and_save(self):
        self.store.create_team("Sun")
        self.store.assign(1, self.charizard)
        problems = self.store.save_spread(1, nature="Timid", level=50, evs={"speed": 252, "special_attack": 252, "hp": 8}, ivs={})
        self.assertTrue(problems and "510" in problems[0])
        self.assertEqual(self.store.save_spread(1, nature="Timid", level=50, evs={"speed": 252, "special_attack": 252, "hp": 4}, ivs={"attack": 0}), [])
        slot = self.store.slot(1)
        self.assertEqual(slot.member.evs, {"speed": 252, "special_attack": 252, "hp": 4})
        self.assertEqual(slot.member.ivs, {"attack": 0})
        self.assertEqual(slot.battle_stats.speed, 167, "100 base, 252 EV, Timid: floor((floor((200+31+63)·0.5)+5)·1.1)")
        self.assertIn("252 Spe", slot.spread_summary)
...
class TestTeamStoreExtras(_TeamStoreCase):
    def test_duplicate_team_copies_slots(self):
        ...
        self.store.assign(1, self.charizard)
        self.store.set_item(1, "charizardite-x")
        self.store.save_spread(1, nature="Adamant", level=50, evs={"attack": 252}, ivs={})
        ...
        self.assertEqual(slot.member.evs, {"attack": 252})
