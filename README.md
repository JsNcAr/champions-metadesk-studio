...
- **590 automated tests** cover the domain rules, repositories, services and the UI. UI tests
...
- **Box.** Add Pokémon by name (forms and Megas normalised), filter by type, stats, tags,
  ...
  CSV.
- **Teams.** Six slot cards with forms, abilities, Tera, items with Champions legality
  guardrails and stat deltas, legal moves ranked by tournament usage, and the Champions
  stat-point spread editor (0–32 per stat, 66 in total). Showdown and Poképaste import and
  export, team health checks, and offensive and defensive coverage grids.
...
- **Calc.** Both directions at once, with each side's best hit and who moves first always in
  ...
  checking. **Rival teams** keep the teams you plan against (saved from Meta, a paste or a
  Poképaste link) and the one you are facing: enter the six species at team preview, and
  each rival is one click from the Defender panel, keeps what the battle reveals, and
  shows up in a Team vs team grid of every pairing.
